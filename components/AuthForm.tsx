  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (type === "sign-up") {
        ...
        // Redirect to dashboard page and refresh session
        router.push("/");
        router.refresh();
      }
    } catch (error) {
    ...
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="auth-form">
          ...
          <Button
            type="submit"
            className="form-submit-button bg-brand hover:bg-brand-100 transition-all rounded-full h-[66px] text-white w-full"
            disabled={isLoading}
          >
